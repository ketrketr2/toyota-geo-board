...
3. 過去30日の日次標準偏差から**±2σ以内は「誤差の範囲（実質横ばい）」**とグレー表示（色を付けない）
...
クエリ一覧は市場に合わせて毎週入れ替わる。`python src/harvest.py` が1サイクル。

**収穫（4経路）**

| 経路 | 中身 | コスト |
|---|---|---|
| Googleサジェスト | 検索窓に実際に打たれている語 | 無料（公式endpoint） |
| Google「関連する質問」 | SERPのPAA。すでに疑問文 | DataForSEO SERP |
| Yahoo!知恵袋 | 生活者が自分の言葉で書いた実質問 | DataForSEO SERP（site:検索） |
| LLM fan-out還流 | AIが回答生成中に内部で投げた派生クエリ | 日次実行の副産物なので無料 |

サジェストとfan-outは名詞句なので、`config/harvest.yaml` の `question_templates` で
生活者の聞き方に変換してから登録する。

**需要スコア（0〜100）**

`検索ボリューム45 + fan-out出現25 + 伸び率20 + UGC出現10`

取れなかったシグナルは**ゼロではなく「未計測」**として扱い、取れた配点だけで正規化する。
そのうえで**証拠が少ないほど割り引く**（`0.55 + 0.45 × 取得できた配点比`）ので、
1経路でたまたま目立っただけのクエリが上位に来ることはない。

**入替のブレーキ**

一気に入れ替えるとスコアの母集団が変わり、動いた理由が施策か母集団か分からなくなる。

- コア枠は定員制（既定60本）
- 1回の入替は最大4本、**需要スコア差8pt以上**でないと動かさない
- 昇格後28日は降格させない
- `priority: high` の判断軸（安全・都市の取り回し・運転支援）は**市場人気が落ちても守る**
- 各テーマ最低4本は残す

**イベントと急上昇 — 週次の入替では間に合わないもの**

需要は平常時のトレンドだけでは動かない。「残クレでアルファードを買うのは危ないらしい」のような
突発的な不安、新型車の発表、税制改正 — こういう出来事が起きた瞬間に聞き方が変わる。
これを需要スコア順の週次入替で待つと、いちばん見たい最初の2週間を逃す。

- `config/events.yaml` の **calendar**（日付が読めるもの：発表会・税制施行・季節）は、
  期間に入ると自動でシードに追加され、関連クエリを先回りで拾う
- **watch**（日付は読めないが起きたら必ず増えるもの：残クレ不安・リコール・受注停止・値上げ）は
  急上昇検知のヒント語として使い、該当すると伸び率に係数がかかる
- 伸び率が閾値（既定1.6倍）を超えたものは、**定員を一時的に超えてでも即日コア枠へ**入れる。
  超過分は翌週の収穫で解消する

**基準コホート — これが時系列を守る肝**

クエリを入れ替えると、施策と関係なく出現率が動く。そこで:

- **30日以上コア枠に居るクエリ＝基準コホート**だけで前日比・先週比を計算する
- 新規クエリは30日間「観察期間」として比較の分母に入れない
- ダッシュボードには**基準コホートのスコア（比較用）**と**全体スコア（実態）**を両方出す

スナップショットの `cohort` フィールドがこれ。`diff.py` は必ずこちらを見る。

**IDは絶対に振り直さない。** 降格しても削除せず `tier` を変えるだけなので、
需要が戻れば同じIDのまま復帰し、過去の時系列とつながる。

```bash
python src/registry.py --migrate     # 旧 core.yaml → registry.yaml（初回のみ）
python src/harvest.py                # 1サイクル回す
python src/harvest.py --date 2026-07-31
```

GitHub Actions では**JST月曜だけ**自動実行する（毎日やるとコア枠が落ち着かないため）。
手動で回したいときは Actions → Run workflow → 「クエリ収穫も実行する」にチェック。
...
config/
  ...
  harvest.yaml      収穫の種・経路・需要スコア配点・昇降格ルール ★クエリ新陳代謝の設定
  events.yaml       イベントカレンダーと監視テーマ（新型車・税制・炎上型）
prompts/
  registry.yaml     全クエリのマスター ★これが唯一の正。IDは絶対に振り直さない
  core.yaml         旧形式。registry.yaml が無いときのフォールバック
src/
  registry.py          クエリレジストリの読み書き・重複判定・tier管理
  harvest.py           クエリ収穫と昇降格（週次）
...
data/fanout/          AIが内部で投げた派生クエリ（次サイクルの候補になる）
data/harvest_log.json 収穫と入替の履歴
...
- **`prompts/registry.yaml` の `id` は絶対に書き換えない。** 文言を直したいときも
  新しいIDで足して古い方を `tier: retired` にする。IDを使い回すと時系列が嘘になる
- 90日を超えたスナップショットは明細（`cells`）が自動削除される（`prune_snapshots`）。
  スコアと因数は残るので履歴グラフには影響しない
- `config/brands.yaml` の `drivers` で `priority: high` にした軸は、必ずコアプロンプトに含める
- 重みの点検は「SNS別評価 → 重みは実測と合っているか」で毎月見る。
  ただし **`weight` の変更は四半期に1回まで**。頻繁に変えるとスコアの時系列が切れる
